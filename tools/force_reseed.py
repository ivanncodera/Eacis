"""
Force reseed the application's DB: deletes seeded users/products and re-inserts seed data.
Run: python tools/force_reseed.py
"""
import sys, os
...
# reuse seed definitions from seeds/seed.py
KITCHEN_PRODUCTS = [
...
with app.app_context():
    print('Dropping seeded products and demo users (customer/seller/admin)')
    # remove products with product_ref in KITCHEN_PRODUCTS
    refs = [p[0] for p in KITCHEN_PRODUCTS]
    Product.query.filter(Product.product_ref.in_(refs)).delete(synchronize_session=False)
    # remove demo users by email
    User.query.filter(User.email.in_(['customer@eacis.ph','seller@eacis.ph','admin@eacis.ph'])).delete(synchronize_session=False)
    from eacis.app import db
    db.session.commit()

    # recreate users and products
